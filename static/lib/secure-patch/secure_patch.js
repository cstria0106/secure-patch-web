 ...
 * @returns {Uint8Array}
 */
export function generate_patch(before, after) {
    ...
    const ret = wasm.generate_patch(ptr0, len0, ptr1, len1);
 ...
 * @returns {Uint8Array}
 */
export function apply_patch(before, patch) {
    const ptr0 = passArray8ToWasm0(before, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('secure_patch_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
